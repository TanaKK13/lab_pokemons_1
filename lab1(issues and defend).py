...
count = 0 
marks = '!@#$%^&*()_+-!№;%:?*-=,./><{}[]" \''
count_chars = sum(i in marks for i in pokemons)
print ('Количество символов в строке без знаков препинания и пробелов:', chr_total - count_chars)
...
pokemons_max_abilities = []
for pokemon in pokemons_list:
        for ability in pokemon ['abilities']:
                count_words = len(ability.split())
                if count_words == res_max_count_words:
                        pokemons_max_abilities.append (ability)
if len(pokemons_max_abilities) == 1:
        print ('Умение, которое содержит больше всего слов:', max_count_words_of_ability)
else:
        print ('Умения, которые содержат максималное количество слов:', pokemons_max_abilities)
